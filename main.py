...
# reports helpers (I/O + cost-basis)
...
# aggregates / per-asset totals
from reports.aggregates import aggregate_per_asset, format_per_asset_totals

# try to import day report from module; if missing, we fallback later
try:
    from reports.day_report import build_day_report_text as _external_build_day_report_text
except Exception:
    _external_build_day_report_text = None
...
def update_ath_key(key: str, live_price: float):
...
def rpc_discover_wallet_tokens(window_blocks: int = None, chunk: int = None):
    ...
    if not contracts:
        try:
            from_addr_txs = fetch_latest_token_txs(limit=1000)
            for t in from_addr_txs or []:
...
def rebuild_open_positions_from_history():
    ...
    def _upd(token_key, signed_amount, price_usd):
        realized = update_cost_basis(pos_qty, pos_cost, token_key, signed_amount, price_usd, eps=EPSILON)
        return realized
...
# ------------------------------------------------------------
# Fallback: build_day_report_text (if reports.day_report not present)
# ------------------------------------------------------------
def _fallback_build_day_report_text():
    date_str = ymd()
    ...
    try:
        ...
        pass
    lines.append(f"\n*Month Net Flow:* ${_format_amount(total_flow)}")
    lines.append(f"*Month Realized PnL:* ${_format_amount(total_real)}")
    return "\n".join(lines)

# pointer used by loops
build_day_report_text = _external_build_day_report_text or _fallback_build_day_report_text
...
def handle_native_tx(tx: dict):
    ...
    realized = update_cost_basis(_position_qty, _position_cost, "CRO", sign * amount_cro, price, eps=EPSILON)
...
def handle_erc20_tx(t: dict):
    ...
    realized = update_cost_basis(_position_qty, _position_cost, key, sign * amount, (price or 0.0), eps=EPSILON)

    try:
        if _nonzero(price):
            ath_key = token_addr if token_addr else symbol
            update_ath_key(ath_key, price)
    except Exception:
        pass
...
# ------------------------------------------------------------
# Month/day helpers for Telegram summaries (already split aggregates)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Dexscreener monitor / discovery / alerts / guard
# ------------------------------------------------------------
...
def _replay_today_cost_basis():
    """Rebuilds today's cost-basis into the runtime state using reports.ledger.replay_cost_basis_over_entries"""
    path = data_file_for_today()
    data = read_json(path, default={"date": ymd(), "entries": []})
    entries = data.get("entries", [])
    try:
        # resets are done inside the helper based on passed dicts
        replay_cost_basis_over_entries(_position_qty, _position_cost, entries, eps=EPSILON)
    except Exception as e:
        log.debug("replay_today_cost_basis failed: %s", e)
