...
def send_telegram(text: str, *, parse_mode: Optional[str] = "HTML", disable_preview: bool = True) -> None:
    ...
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        # In non-configured environments, just log the message to stdout
        print("[TELEGRAM]", text)
        return

    # Prepare content
    if parse_mode == "HTML" and text:
        # Always escape to prevent parse errors; we don't add any tags here
    ...
    for idx, chunk in enumerate(_split_chunks(safe_text), 1):
        if idx > 1:
            time.sleep(0.15)  # tiny pacing between chunks
